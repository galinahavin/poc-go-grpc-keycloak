...
.DEFAULT_GOAL := help
.PHONY: greet blog calculator ecommerce help
project := greet calculator blog ecommerce
...
ecommerce: $@ ## Generate Pbs and build for ecommerce
...
clean: clean_greet clean_calculator clean_blog clean_ecommerce ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_ecommerce: ## Clean generated files for ecommerce
	${RM_F_CMD} ecommerce/${PROTO_DIR}/*.pb.go
